...
def get_image_files():
    """
    Find all supported image files from the
...
def load_checkpoint():
    """
    Load progress from checkpoint.json.

    If no checkpoint exists, start from image 1.
    """
...
def save_checkpoint(
    ...
    """
    Save progress after every processed image.
    """
...
def log_failed_image(
    ...
    """
    Record failed images so they can be reviewed later.
    """
...
def process_one_image(
    ...
    """
    Process one dataset image using Jinwen's pipeline
    and save the final image while preserving the
    original class/folder structure.
    """
...
def process_dataset():
    ...
    print(
        "JINWEN FULL DATASET PREPROCESSING"
    )
